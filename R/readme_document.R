...
#'
#' Only a generated `index.md` is dropped. Every one this format writes opens
#' with a marker comment, the way roxygen2 stamps the files it generates, and
#' a file without that marker is left where it is with a warning.
...
#
# Provenance has to survive between renders, and nothing else here does: the
# post-processor sees a file it may or may not have written, with no way to ask
# which. The marker is the same answer roxygen2 gives for the same problem --
# it stamps every file it generates and refuses to touch one that is not
# stamped -- and it costs a comment that no markdown renderer shows.
#
# The comparison against `README.md` runs on the unmarked text, so the marker
# never decides whether the two files differ; it is added on the way out.
